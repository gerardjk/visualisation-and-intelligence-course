"""Build the Week 2 deck: Choosing Visual Forms. Encoding, Tidy Data, and the Visual Vocabulary."""
...
OUTPUT = ROOT / "merged_course" / "classes" / "02-visual-forms" / "Choosing-Visual-Forms.pptx"
ORIGINALS = ROOT / "merged_course" / "shared" / "assets" / "originals"


def original_slide(deck, title, image, credit):
    """Full-width original artefact beside-the-reproduction pattern."""
    slide = deck.content(title, credit)
    add_picture_contain(slide, ORIGINALS / image,
                        Inches(0.7), Inches(1.5), Inches(11.9), Inches(5.4))
    return slide
COVER = ROOT / "merged_course" / "shared" / "assets" / "course_cover.png"


def build():
    deck = Deck("CHOOSING VISUAL FORMS", "Choosing Visual Forms")

    # Shared, semester-level cover copied from cover.pdf.
    slide = deck.blank()
    slide.shapes.add_picture(
        str(COVER), 0, 0, width=deck.prs.slide_width, height=deck.prs.slide_height
    )
    ...
    # 3b. Bertin's variables (the grammar the ranking orders)
    deck.figure_slide(
    ...
    #: channel sprint: twelve figures, name the dominant channel
    deck.statement(
        "The channel sprint",
        "Twelve charts · 40 seconds each. Name the channel doing the work.",
        "Position, length, angle, area, colour value, colour hue, one dominates each chart. Write your call; we debrief three.",
    )
    SPRINT = [
        ("fig-colourwheel-output-1.png", "The colour wheel"),
        ("fig-cmb-map-output-1.png", "The cosmic microwave background"),
        ("fig-ozone-output-1.png", "The ozone hole"),
        ("fig-molecule3d-output-1.png", "A molecule in 3D"),
        ("fig-dna-output-1.png", "The DNA double helix"),
        ("fig-ribbon-output-1.png", "A protein ribbon"),
        ("fig-orbitals-output-1.png", "Atomic orbitals"),
        ("fig-cosmicweb-output-1.png", "The cosmic web"),
        ("fig-projection-output-1.png", "The map projection problem"),
        ("fig-periodic-output-1.png", "The periodic table"),
        ("fig-codonwheel-output-1.png", "The genetic code wheel"),
        ("fig-wallace-output-1.png", "The Wallace line"),
    ]
    for fname, label in SPRINT:
        slide = deck.blank()
        add_text(slide, label, Inches(0.55), Inches(0.28), Inches(12.2), Inches(0.5),
                 size=24, color=NAVY, bold=True, align=PP_ALIGN.CENTER)
        add_picture_contain(slide, FIGURES / fname,
                            Inches(0.9), Inches(0.95), Inches(11.5), Inches(6.1))
    ...
    # 11a. Minard original
    original_slide(
        deck, "The original: Minard, 1869",
        "minard-1869.png",
        "Charles Joseph Minard, Carte figurative des pertes successives en hommes "
        "de l’Armée Française dans la campagne de Russie 1812–1813 (1869). Public domain.",
    )

    # 11: spatial + flow
    deck.figure_slide(
        "Spatial and flow: movement carries the claim",
    ...
    # 12d-0: the 1874 plate
    original_slide(
        deck, "The original: the Statistical Atlas, 1874",
        "statistical-atlas-1874-pyramid.jpg",
        "Chart showing the distribution by age and sex of deaths, Statistical Atlas "
        "of the United States, Francis A. Walker (1874). Public domain.",
    )
    ...
    add_bullets(slide, [
        "Open visual_forms_activities.ipynb in VS Code; an AI assistant is available but not required.",
