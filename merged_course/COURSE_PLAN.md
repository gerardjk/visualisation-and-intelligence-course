...
```text
Question → Evidence → Field types / roles → Required comparison →
Visual task → Visual form → Interaction → Deployment → Verification →
Disclosure → Review → Defence
```

The FT Visual Vocabulary is the shared language for visual-task classification
and chart selection. Measurement type and semantic role constrain which
comparisons are valid; the audience's question determines which valid task the
visualisation must support. Students work in notebooks with an AI assistant available;
...
|---|---:|---|---|---|
| **A1. Critique and Repair portfolio** | 30% | Individual | Part A supervised & AI-restricted; Part C AI-integrated with a supplied-text fallback | Released in Seeing Data; supervised window at the start of the fourth live session (Week 5, 27 August); due Friday 28 August |
...
One is the primary; the other two are supporting.
The pool adapts twelve Week 2 chart-card genres using exact published figures
from varied sources and domains. No datasets are supplied: reconstructing
and documenting the visible data is part of the assessment.
Source artefacts may repeat across students, but every submission is individual.
Part A: critiques — the full 9-field critique of the primary
(what the chart shows vs what it claims, ending with a repaired caption),
drafted in a supervised AI-restricted window at the start of Week 5 (27 August), where
the primary's pool ID is written on the sheet (META_1 must match); plus a
full 9-field critique of each supporting artefact, completed outside the
supervised component. Part B: repairs —
...
merged_course/
  ...
  scripts/                    ← notebook, pool and figure builders (regenerable)
  classes/
    01-seeing-data/
      ...
      beamer/<deck>.tex       ← canonical deck (compile with tectonic/lualatex)
      notebooks/
...
Decks are LaTeX/Beamer (the python-pptx pipeline is retired). Class-02 deck
figures come from the atlas freeze
(`quarto-book/_freeze/atlas/figure-html/`) plus generated course figures;
originals live in `shared/assets/originals/` with `PROVENANCE.md` entries.
